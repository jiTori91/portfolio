...
$(document).ready(function () {
	var article = $('.content_area ul .article');  //모든 질문 답변 리스트
	//article.find('.a').slideUp(100); // 모든 답변 닫아라
    article.find('.q').find('span').html('<i class="fas fa-chevron-down"></i>');
	...
	$('.content_area ul .article .trigger').click(function(e){  // 각각의 질문을 클릭하면
        ...
        if(myArticle.hasClass('hide')){   //클릭한 해당 리스트의 상태가 hide냐?? 답변이 닫혀있냐??
            ...
            myArticle.find('.q').find('span').html('<i class="fas fa-chevron-up"></i>');
        } else {  // 'show' 답변이 열려있냐??
            myArticle.removeClass('show').addClass('hide');  // hide로 바꾼다 
            myArticle.find('.a').slideUp(100);  //해당 리스트의 답변을 닫아라  
            myArticle.find('.q').find('span').html('<i class="fas fa-chevron-down"></i>');
        }  
  });
  ...
  $('.all').toggle(function(e){
        ...
        article.find('.q').find('span').html('<i class="fas fa-chevron-up"></i>');
