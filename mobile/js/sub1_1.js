       ...
       success: function(data){ //json파일에 연결이 완료되면 자동으로 호출되는 함수
           ...
           function dataPrint(arr){  //검색된 또는 전체 상품리스트 출력
                   ...
                   for(var i in arr){
                       ...
                       var $Name = arr[i].Name;
                       var $EngName = arr[i].EngName;
                       var $Showday = arr[i].Showday;
                       var $pop_showtime = arr[i].pop_showtime;
                       var $pop_runtime = arr[i].pop_runtime;
                    ...
                    txt+='<li>';
                        txt+='<a href="#" class="pop_open">';
                            ...
                            txt+='</dl>';
                            txt+='<dl>';
                                txt+='<dt><span>일</span><span>자</span></dt>';
                                txt+='<dd>'+$Showday+'</dd>';
                                txt+='<dt>공연시간</dt>'  
                                txt+='<dd>'+$pop_showtime+'</dd>'
                                txt+='<dt>러닝타임</dt>';
                                txt+='<dd>'+$pop_runtime+'</dd>';
                            txt+='</dl>';
                            txt+='</div>';
                        txt+='</a>';
                    txt +='</li>';
           ...
           if(value){
                ...
                if(newArray.length!=0){ //검색된 상품이 배열에 있으면...
                  dataPrint(newArray);
                    ...
                    $('.product_list').html(' <a href="./sub1_1.html" style="text-align: center; margin: 30px 0"><button type="button"  style="padding: 10px 20px; background: #231816; color: #fff; border: none; border-radius: 30px;">전체공연보기</button></a><span style="display:block;text-align:center; font-size:15px; color:red; padding-bottom: 50px;">검색된 상품이 없습니다</span>');
                }
                ...
                if($('#title').val()){ //검색어가 있으면
                    ...
                    if(newArray.length!=0){
                        dataPrint(newArray);
                          ...
                          $('.product_list').html('<span style="display:block;text-align:center; font-size:15px; color:red;padding-bottom: 50px;">검색된 상품이 없습니다</span>');
                    }
        ...
        function popchange(obj){
            ...
            pop_txt+='<dl>'
            pop_txt+= '<dt>일자</dt>' 
            pop_txt+= '<dd>'+obj.Showday+'</dd>'
            pop_txt+='</dl>'
    
    
            pop_txt+='<dl>'
            pop_txt+= '<dt>공연시간</dt>'  
            pop_txt+= '<dd>'+obj.pop_showtime+'</dd>'
            pop_txt+='</dl>'
